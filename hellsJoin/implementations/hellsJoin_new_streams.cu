...
__global__
void compare_kernel_ipt_(tuple input, int *output, size_t etpw, tuple *compareTuples) {
...
void thread_work(tuple new_tuple, Stream stream){

	cudaStream_t new_stream;
	cudaStreamCreate(&new_stream);
	int *compare_output;
    CUDA_SAFE(cudaHostAlloc((void **)&compare_output, sizeof(int) * ((etpw / 32) + 1), 0));

	if (stream == stream1) {
		compare_kernel_ipt_<<<blocksize, gridsize,0,new_stream>>>(new_tuple, compare_output,  etpw, compareTuples_s2_comp);
		//add_new_tuple_device<<<1, 1,0,new_stream>>>(new_tuple, stream1, etpw, compareTuples_s1_comp);
	} else {
		compare_kernel_ipt_<<<blocksize, gridsize,0,new_stream>>>(new_tuple, compare_output,  etpw, compareTuples_s1_comp);
		//add_new_tuple_device<<<1, 1,0,new_stream>>>(new_tuple, stream2, etpw, compareTuples_s2_comp);
	}
	// Create new memory
	// Move to new memory

	// Start interpretation of prev tuple while execution
	write_result(interprete(new_tuple, compare_output, stream));
}
...
void startManualTest(){
    ...
    for (int k = 0; i < 10  && j < 14; k++) {
		while (records0[i].timestamp == k && i < 10) {
			...
			pool.enqueue([new_tuple] (){thread_work(new_tuple, stream1);});
			// Save prev setup.
		...
		while (records1[j].timestamp == k && j < 14) {
			...
			pool.enqueue([new_tuple] (){thread_work(new_tuple, stream2);});
...
void startFileTest(std::string filename1, std::string filename2, int rows){;
    ...
    for (int k = 0; i < rows  && j < rows; k++) {
		while (records0[i].timestamp == k && i < rows) {
			...
			compare_kernel_ipt_<<<blocksize, gridsize>>>(new_tuple, compare_output_s1,  etpw, compareTuples_s2_comp);
		...
		while (records1[j].timestamp == k && j < rows) {
			...
			compare_kernel_ipt_<<<blocksize, gridsize>>>(new_tuple, compare_output_s2,  etpw, compareTuples_s1_comp);
