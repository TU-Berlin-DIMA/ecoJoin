...
void worker_thread(tuple input, int *bitmap, Stream stream) {
	write_result(interprete(input, bitmap, stream ));
}
...
void startFileTest(std::string filename1, std::string filename2, int rows){;
    ...
    for (int k = 0; i < rows  && j < rows; k++) {
		while (records0[i].timestamp == k && i < rows) {
			...
			if (i!=0 || j !=0)
				//thr = std::thread(worker_thread, tuple_prev, compare_output_prev, stream_prev);
