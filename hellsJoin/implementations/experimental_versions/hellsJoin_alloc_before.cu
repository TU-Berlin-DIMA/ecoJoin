...
void startFileTest(std::string filename1, std::string filename2, int rows){;
    ...
    for (int k = 0; i < rows  && j < rows; k++) {
		while (records0[i].timestamp == k && i < rows) {
			...
			int * compare_output_new;
			CUDA_SAFE(cudaHostAlloc((void **)&compare_output_new, sizeof(int) * ((etpw / 32) + 1), 0));
			compare_kernel_ipt<<<blocksize, gridsize,0 ,cudastream1>>>(new_tuple, compare_output_new,  etpw, compareTuples_s2_comp);
		...
		while (records1[j].timestamp == k && j < rows) {
			...
			int * compare_output_new;
			CUDA_SAFE(cudaHostAlloc((void **)&compare_output_new, sizeof(int) * ((etpw / 32) + 1), 0));
			compare_kernel_ipt<<<blocksize, gridsize, 0, cudastream3>>>(new_tuple, compare_output_new,  etpw, compareTuples_s1_comp);
