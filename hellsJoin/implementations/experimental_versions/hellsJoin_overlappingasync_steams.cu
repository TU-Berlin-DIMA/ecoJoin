...
void startFileTest(std::string filename1, std::string filename2, int rows){;
	...
	cudaStream_t cudastream1[6];
	cudaStream_t cudastream2[6];
	for (int i = 0;i <6;i++) {
		cudaStreamCreate(&cudastream1[i]);
		cudaStreamCreate(&cudastream2[i]);
	}
	int cs_c1 = 0;
	int cs_c2 = 0;
    ...
    for (int k = 0; i < rows  && j < rows; k++) {
		while (records0[i].timestamp == k && i < rows) {
			...
			compare_kernel_ipt<<<blocksize, gridsize, 0,cudastream1[cs_c1]>>>(new_tuple, compare_output_s1,  etpw, compareTuples_s2_comp);
			if (++cs_c1 == 6)
				cs_c1 = 0;
			add_new_tuple_device<<<1, 1, 0, cudastream1[cs_c1]>>>(new_tuple, stream1, etpw, compareTuples_s1_comp);
			if (++cs_c1 == 6)
				cs_c1 = 0;
			...
			for (int k = 0;k <6;k++) 
				cudaStreamSynchronize(cudastream2[k]);
		...
		while (records1[j].timestamp == k && j < rows) {
			...
			compare_kernel_ipt<<<blocksize, gridsize,0,cudastream2[cs_c2]>>>(new_tuple, compare_output_s2,  etpw, compareTuples_s1_comp);
			if (++cs_c2 == 6)
				cs_c2 = 0;
			add_new_tuple_device<<<1, 1, 0,cudastream2[cs_c2]>>>(new_tuple, stream2, etpw, compareTuples_s2_comp);
			if (++cs_c2 == 6)
				cs_c2 = 0;
			...
			for (int k = 0;k <6;k++) 
				cudaStreamSynchronize(cudastream1[k]);
