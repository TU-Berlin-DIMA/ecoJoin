...
__device__ inline int bindex(int b) { return b / (sizeof(unsigned) * 8); }

__device__ inline int boffset(int b) { return b % (sizeof(unsigned) * 8); }
//int boffset(int b) { return b >> 5; }

__device__ inline void set_bit(int b, unsigned* array)
{
    array[bindex(b)] |= (1 << boffset(b));
}

__device__ inline void clear_bit(int b, unsigned* array)
{
    array[bindex(b)] &= ~(1 << (boffset(b)));
}

__device__ inline bool get_bit(int b, unsigned* array)
{
 ...
 */
__device__ inline long r_get_tns(int generate_tuples_R, int r_iterations, int rate_R, int r, long* r_t_ns)
{
    if (r_iterations > 1) {
...
__device__ inline long s_get_tns(int generate_tuples_S, int s_iterations, int rate_S, int s, long* s_t_ns)
{
    if (s_iterations > 1) {
 ...
 */
__global__ void compare_kernel_new_s_hj(
    int s_processed,
 ...
 */
__global__ void cleanup_r(int s_processed,
    int* output_buffer, int outsize_mask,
    int* a, int* b, int* x, int* y,
    long* s_ts, long* r_ts,
    int generate_tuples_S, int generate_tuples_R,
...
__global__ void compare_kernel_new_r_hj(
    int r_processed,
    int* output_buffer, int outsize_mask,
    int* a, int* b, int* x, int* y,
    long* s_ts, long* r_ts,
 ...
 * used threadnumber == ht_size
 */
__global__ void cleanup_s(int r_processed,
    int* output_buffer, int outsize_mask,
    int* a, int* b, int* x, int* y,
    long* s_ts, long* r_ts,
    int generate_tuples_S, int generate_tuples_R,
