...
long get_current_ns(master_ctx_t* ctx, long t)
{
    unsigned long range = 1.e9 / ctx->rate_R;
    //unsigned long range = 1.e8 / ctx->rate_R;
    long z = range; //(random() % range);
    return t + z;
}

unsigned get_current_ns(master_ctx_t* ctx, unsigned t)
{
    unsigned long range = 1.e9 / ctx->rate_R;
    //unsigned long range = 1.e8 / ctx->rate_R;
    unsigned z = range; //(random() % range);
    return t + z;
}
/**
...
void generate_data(master_ctx_t* ctx)
{
    ...
    long t = 0; /* "current time" in nano-seconds */
    ...
    /* allocate memory */
    if (ctx->processing_mode == ht_gpu_mode) {
        ...
        fprintf(stderr, "processing modes not allowed\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (!ctx->linear_data) { // random data
		...
		*/
            ...
            ctx->R.y[i] = random() % ctx->int_value_range;
        }
		...
		if (j % 2 == 0) {
            ...
            ctx->S.b[i] = random() % ctx->float_value_range;
        }
    } else { // linear data
