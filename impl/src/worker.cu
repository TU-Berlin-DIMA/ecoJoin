...
void start_batch(master_ctx_t* ctx, worker_ctx_t* w_ctx)
{
    ...
    w_ctx->stats.runtime = std::chrono::duration_cast<std::chrono::nanoseconds>(w_ctx->stats.end_time - w_ctx->stats.start_time).count();
...
void init_worker(worker_ctx_t* w_ctx, master_ctx_t* ctx)
{
    ...
    if (w_ctx->processing_mode == gpu_mode || w_ctx->processing_mode == atomic_mode) {
    ...
    if (w_ctx->processing_mode == ht_cpu1_mode || w_ctx->processing_mode == ht_cpu2_mode || w_ctx->processing_mode == ht_cpu3_mode || w_ctx->processing_mode == ht_cpu4_mode) {
...
void process_s_gpu(worker_ctx_t* w_ctx)
{
    ...
    if (r_processed - r_first > 0) {
        ...
        if (w_ctx->range_predicate) {
            compare_kernel_new_s_range<<<w_ctx->gpu_gridsize, w_ctx->gpu_blocksize>>>(w_ctx->gpu_output_buffer,
...
void process_r_gpu(worker_ctx_t* w_ctx)
{
    ...
    if (s_processed - s_first > 0) {
        ...
        /* Start kernel */
        if (w_ctx->range_predicate) {
            compare_kernel_new_r_range<<<w_ctx->gpu_gridsize, w_ctx->gpu_blocksize>>>(w_ctx->gpu_output_buffer,
...
void process_s_gpu_atomics(worker_ctx_t* w_ctx)
{
    ...
    if (r_processed - r_first > 0) {
        if (w_ctx->range_predicate) {
            compare_kernel_new_s_atomics_range<<<w_ctx->gpu_gridsize, w_ctx->gpu_blocksize>>>(w_ctx->gpu_output_buffer,
...
void process_r_gpu_atomics(worker_ctx_t* w_ctx)
{
    ...
    if (s_processed - s_first > 0) {
        if (w_ctx->range_predicate) {
            compare_kernel_new_r_atomics_range<<<w_ctx->gpu_gridsize, w_ctx->gpu_blocksize>>>(w_ctx->gpu_output_buffer,
                w_ctx->gpu_output_buffer_size / 8,
                &(w_ctx->S.a[s_first]), &(w_ctx->S.b[s_first]),
                &(w_ctx->R.x[r_processed]), &(w_ctx->R.y[r_processed]),
                s_processed - s_first,
...
void expire_outdated_tuples_(worker_ctx_t* w_ctx, master_ctx_t* ctx)
{
    const int s_processed = *(w_ctx->s_processed);
    const int r_processed = *(w_ctx->r_processed);
