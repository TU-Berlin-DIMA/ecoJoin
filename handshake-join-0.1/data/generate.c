...
generate_data (master_ctx_t *ctx)
{
    ...
    ctx->R.t = alloc_onnode ((ctx->num_tuples_R + 1) * sizeof (*ctx->R.t), 0);
    ctx->R.x = alloc_onnode ((ctx->num_tuples_R + 1) * sizeof (*ctx->R.x), 0);
    ctx->R.y = alloc_onnode ((ctx->num_tuples_R + 1) * sizeof (*ctx->R.y), 0);
    //ctx->R.z = alloc_onnode ((ctx->num_tuples_R + 1) * sizeof (*ctx->R.z), 0);
    ...
    /* FIXME: Consider NUMA here */
    ctx->S.t = alloc_onnode ((ctx->num_tuples_S + 1) * sizeof (*ctx->S.t), 0);
    ctx->S.a = alloc_onnode ((ctx->num_tuples_S + 1) * sizeof (*ctx->S.a), 0);
    ctx->S.b = alloc_onnode ((ctx->num_tuples_S + 1) * sizeof (*ctx->S.b), 0);
    //ctx->S.c = alloc_onnode ((ctx->num_tuples_S + 1) * sizeof (*ctx->S.c), 0);
    //ctx->S.d = alloc_onnode ((ctx->num_tuples_S + 1) * sizeof (*ctx->S.d), 0);
