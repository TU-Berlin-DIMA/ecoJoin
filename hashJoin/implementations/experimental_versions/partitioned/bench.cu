...
int main() {
	int k = pow(2,8); // Input size
	unsigned buckets = k * 2 ; // Table Size (2* entries)
	//unsigned buckets = k; // Table Size (2* entries)
	...
	// (1)
	cudaHostAlloc((void**)&keys_p_h,  k*sizeof(int), cudaHostAllocDefault);;
	cudaHostAlloc((void**)&vals_p_h,  k*sizeof(int), cudaHostAllocDefault);;
	cudaHostAlloc((void**)&keys_b_h,  k*sizeof(int), cudaHostAllocDefault);;
	cudaHostAlloc((void**)&vals_b_h,  k*sizeof(int), cudaHostAllocDefault);;
	...
	std::cout <<  "p range " << tmp1[0] << " " << tmp1[k-1]<< "\n";
	std::cout <<  "b range " << tmp2[0] << " " << tmp2[k-1]<< "\n";
	...
	indexes1.clear();
	indexes2.clear();
	int outputsize = (tmp2[k-1] - tmp1[0]+1)*3;
	...
	double gb = (double) k*4*sizeof(int) /1024/1024/1024;
	long total_b = 0;
	long total_p = 0;
	...
	for (int j = 0; j < runs; j++){
		...
		{	
		START_M()
		build_linprobe_shared<<<4, 128, 4*128*sizeof(int)*2, stream>>>(k, k, keys_b_h, vals_b_h);
		...
		/*{
		START_M()
		probe_linprobe_lsb<<<32*128,1024,0,stream>>>(k, keys_p_h, vals_p_h, content, buckets, output_h, current_d);
		CUDA_SAFE(cudaStreamSynchronize(stream));
		END_M()
	...
	std::cout << "Build: nsec " << nsec_b << " gb " << gb << " gb/s " << (4*k*sizeof(int))/nsec_b<<"\n";
